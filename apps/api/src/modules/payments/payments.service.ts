...
const TRANSITIONS: Record<PaymentStatus, PaymentStatus[]> = {
  DRAFT: [PaymentStatus.SUBMITTED],
  SUBMITTED: [PaymentStatus.APPROVED, PaymentStatus.DISPUTED],
  APPROVED: [PaymentStatus.PAID, PaymentStatus.DISPUTED],
  PAID: [],
  DISPUTED: [PaymentStatus.DRAFT],
};
@Injectable()
export class PaymentsService {
  ...
  async list(organizationId: string, contractId: string) {
    ...
    return (
      await this.prisma.paymentMilestone.findMany({
        where: { contractId },
        orderBy: { dueDate: "asc" },
      })
    ).map(this.toResponse);
  ...
  async create(
    ...
    return this.toResponse(
      await this.prisma.paymentMilestone.create({
        data: {
          contractId,
          title: dto.title,
  ...
  async transition(
    ...
    if (!TRANSITIONS[milestone.status].includes(status))
  ...
  private toResponse(milestone: PaymentMilestone) {
    return { ...milestone, amountKobo: milestone.amountKobo.toString() };
  }
}
