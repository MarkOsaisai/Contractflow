...
export class LifecycleService {
  ...
  async stages(organizationId: string, contractId: string) {
    const contract = await this.contractForOrganization(
      organizationId,
      contractId,
    );
  ...
  async complete(
    ...
    if (!LIFECYCLE_MANAGERS.includes(role)) {
      throw new BadRequestException(
        "Your role cannot advance a contract lifecycle",
      );
    }
    const contract = await this.contractForOrganization(
      organizationId,
    ...
    if (contract.stage !== stage)
      throw new BadRequestException("Only the current stage can be completed");

    const nextStage = STAGES[STAGES.indexOf(stage) + 1];
    if (!nextStage)
      throw new BadRequestException("Contract lifecycle is already complete");
    const updated = await this.prisma.contract.update({
      where: { id: contractId },
      data: {
        stage: nextStage,
        status:
          nextStage === ContractStage.CLOSED
    ...
    await this.prisma.$transaction([
      this.prisma.activity.create({
        data: {
          ...
          message: `Completed ${stage}; advanced to ${nextStage}.`,
        },
      }),
  ...
  private async contractForOrganization(
    organizationId: string,
    contractId: string,
  ) {
